...
var BIP32 = function(bytes) {
    ...
    if( typeof bytes === "string" ) {
        ...
        var hash = Bitcoin.crypto.sha256(Bitcoin.crypto.sha256(bytes, { asBytes: true }), { asBytes: true });
...
BIP32.prototype.init_from_bytes = function (bytes) {
    ...
    var normbytes = Array.apply([], bytes);
    ...
    if (is_private && key_bytes[0] == 0) {
        ...
        this.eckey = Bitcoin.ECKey.fromBuffer(key_bytes.slice(1, 33));
        //this.eckey = new Bitcoin.ECKey(key_bytes.slice(1, 33));
        //this.eckey.setCompressed(true);
        this.ecpubkey = this.eckey.pub;
        var ecparams = Bitcoin.sec("secp256k1");
        var pt = ecparams.g.multiply(this.eckey.D);
        this.eckey.pub = pt;
        ...
        //this.eckey = new Bitcoin.ECPubKey.fromBuffer(key_bytes);
        this.ecpubkey = new Bitcoin.ECPubKey.fromBuffer(key_bytes);
...
BIP32.prototype.build_extended_public_key = function() {
    ...
    // Parent fingerprint
    this.extended_public_key = this.extended_public_key.concat(Array.apply([], this.parent_fingerprint));
    ...
    // Chain code
    this.extended_public_key = this.extended_public_key.concat(Array.apply([], this.chain_code));

    var normbytes = Array.apply([], this.ecpubkey.toBuffer());

    // Public key
    this.extended_public_key = this.extended_public_key.concat(normbytes);
}

BIP32.prototype.extended_public_key_string = function(format) {
    if( format === undefined || format === "base58" ) {
        var hash = Bitcoin.crypto.sha256(Bitcoin.crypto.sha256(this.extended_public_key, { asBytes: true }), { asBytes: true });
        var checksum = hash.slice(0, 4);
        var data = this.extended_public_key.concat(checksum);
        return Bitcoin.base58.encode(data);
    } else if( format === "hex" ) {
        return Bitcoin.crypto.util.bytesToHex(this.extended_public_key);
    } else {
...
BIP32.prototype.build_extended_private_key = function() {
    ...
    this.extended_private_key = this.extended_private_key.concat(this.eckey.toBuffer());
}

BIP32.prototype.extended_private_key_string = function(format) {
    if( format === undefined || format === "base58" ) {
        var hash = Bitcoin.crypto.sha256(Bitcoin.crypto.sha256(this.extended_private_key, { asBytes: true }), { asBytes: true });
        var checksum = hash.slice(0, 4);
        var data = this.extended_private_key.concat(checksum);
        return Bitcoin.base58.encode(data);
    } else if( format === "hex" ) {
        return Bitcoin.crypto.util.bytesToHex(this.extended_private_key);
    } else {
        throw new Error("bad format");
    }
}
...
BIP32.prototype.derive_child = function (i) {
    ...
    if (this.has_private_key) {
        var buff = null;
        //var mybuffer = new Buffer(37);
        //convert this to buffer
        if (use_private) {
            buff = this.eckey.toBuffer();
            //data = this.eckey.concat(ib);
            buff = buff.writeUInt8(0, 0);
            //data = [0].concat(this.eckey.priv.toByteArrayUnsigned()).concat(ib);
        } else {
            //data = this.eckey.pub.getEncoded(true).concat(ib);
            buff = this.ecpubkey.toBuffer();
        }

        buff.writeUInt8((i >> 24) & 0xff, buff.length - 1);
        buff.writeUInt8((i >> 16) & 0xff, buff.length - 1);
        buff.writeUInt8((i >> 8) & 0xff, buff.length - 1);
        buff.writeUInt8(i & 0xff, buff.length - 1);

        //var nativeData = new Uint8Array(data);
        //var nativeCC = new Uint8Array(this.chain_code)

        //var j = new jsSHA(Bitcoin.crypto.util.bytesToHex(data), 'HEX');
        //var hash = j.getHMAC(Bitcoin.crypto.util.bytesToHex(this.chain_code), "HEX", "SHA-512", "HEX");

        var hmac512 = new Bitcoin.crypto.HmacSHA512(buff, this.chain_code);

        var il = new BigInteger(hmac512.slice(0, 64), 16);
        var ir = hmac512.slice(64, 128);
        ...
        var ir = Bitcoin.crypto.util.hexToBytes(hash.slice(64, 128));
...
function decompress_pubkey(key_bytes) {
    ...
    x.fromString(Bitcoin.crypto.util.bytesToHex(key_bytes.slice(1, 33)), 16);
